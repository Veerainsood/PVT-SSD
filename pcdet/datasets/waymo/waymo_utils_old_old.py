...
def convert_range_image_to_point_cloud(frame, range_images, camera_projections, range_image_top_pose, ri_index=(0, 1)):
    ...
    for c in calibrations:
        ...
        for cur_ri_index in ri_index:
            ...
            inclinations.append(beam_inclinations.numpy())   # convert to ndarray [N]
            extrinsics.append(extrinsic)                     # ndarray [4,4]
...
def save_lidar_points(frame, cur_save_path,info, use_two_returns=True):
    ...
    points, cp_points, points_in_NLZ_flag, points_intensity, points_elongation, extrinsics, inclinations = convert_range_image_to_point_cloud(
    ...
    info['inclinations'] = inclinations
    ...
    for cnt, data in enumerate(dataset):
        ...
        if update_info_only and sequence_infos_old is not None:
            ...
            num_points_of_each_lidar = save_lidar_points(
                frame, cur_save_dir / ('%04d.npy' % cnt), info=info,use_two_returns=use_two_returns
            )
